    ...
    supabase.auth.getSession().then(async ({ data: { session } }) => {
      ...
      if (session?.user) {
        const { data: profile } = await supabase
          .from('profiles')
          .select('business_name, owner_name, auto_pilot')
    ...
    <div className="min-h-screen bg-[#fafafa] font-sans pb-20">
      ...
      <div className="max-w-2xl mx-auto pt-12 px-4">
        ...
        <form onSubmit={handleSave} className="bg-white p-8 md:p-10 rounded-[2.5rem] border border-gray-100 shadow-[0_10px_30px_rgba(0,0,0,0.03)]">
          ...
          {/* NEW: Auto-Pilot Premium Feature Toggle */}
          <div className="p-6 bg-indigo-50 rounded-[2rem] border border-indigo-100 mb-10">
